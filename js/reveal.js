    ...
    function displayRole(role) {
            ...
            if (allSpies.length > 0) {
                infoHTML += `
                    <div class="spy-list">
                        ...
                        <ul>
                            ${allSpies.map(spy => `<li><strong>${spy.playerName}</strong>${spy.isBlind ? ' (Espião Cego)' : ''}</li>`).join('')}
                        </ul>
                    </div>
                `;
            }
        ...
        if (role.knowsSpies && !role.isBlind && role.faction === 'spy') {
            ...
            if (otherSpies.length > 0) {
                infoHTML += `
                    <div class="spy-list">
                        ...
                        <ul>
                            ${otherSpies.map(spy => {
                                let note = '';
                                if (spy.thumbsUp) note = ' (estende polegar)';
                ...
                const blindSpyExists = roles.some(r => r.isBlind && r.faction === 'spy');
                if (blindSpyExists) {
            ...
            if (role.knowsOtherDesertor && role.otherDesertorIndex !== undefined) {
                ...
                infoHTML += `
                    <div class="spy-list">
                        ...
                        <ul>
                            <li><strong>${otherDesertor.playerName}</strong> (${otherDesertor.faction === 'spy' ? 'Espião' : 'Resistência'})</li>
                        </ul>
                    </div>
                `;
            }
